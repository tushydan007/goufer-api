...
class TestCreateDocument:
    def test_if_user_is_anonymous_returns_401(self, api_client):
        ...
        response = api_client.post('/api/v1/main/documents/', {'document_type': 'Entertainment', 'document_number': 'Entertainment Description', 'document_of_expertise': '', 'uploaded_at': '2024-05-30T22:30:57.768126Z', 'is_verified': False})
        
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        
        
        
    def test_if_user_is_authenticted_but_sends_invalid_data_returns_400(self, api_client):
        
        api_client.force_authenticate(user=User(is_staff=False))
        
        response = api_client.post('/api/v1/main/documents/', {})
        
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        
        
    # def test_if_user_is_authenticated_and_sends_valid_data_returns_201(self, api_client):
        
    #     api_client.force_authenticate(user=User(is_staff=False))
        
    #     response = api_client.post('/api/v1/main/documents/', {'document_type': 'Entertainment', 'document_number': 'Entertainment Description', 'document_of_expertise': 'This is making the test fail', 'uploaded_at': '2024-05-30T22:30:57.768126Z', 'is_verified': False})
